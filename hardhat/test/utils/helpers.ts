...
export const approveAll = async (signer: Signer | JsonRpcSigner, address: string, tokenContract: Contract): Promise<void> => {
  const approval = await tokenContract.connect(signer).approve(address, ethers.constants.MaxInt256);
  await approval.wait();
};

export const getDefaultFnftConfig = (address: string, amount: number): IConfig => {
  const config = {
